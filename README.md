...
Сервис для работы с фильмами и оценками пользователей. Позволяет составлять рейтинги, добавлять фильмы в избранное и управлять списком друзей.
...
* **`friendship`** — таблица для хранения односторонних дружеских связей. Если пользователь A добавляет пользователя B в друзья, запись `(user_id = A, friend_id = B)` создаётся, но обратная запись не добавляется.
* **`film_like`** — таблица для хранения лайков, поставленных пользователями фильмам. Содержит временную метку `created_at`.
...
Представленные ниже запросы соответствуют реальной реализации в DAO-классах (`FilmDbStorage`, `UserDbStorage`, `GenreDbStorage`, `MpaDbStorage`).
